...
CREATE TABLE IF NOT EXISTS leads (
  ...
  outcome TEXT DEFAULT 'Pending' CHECK (outcome IN ('Pending', 'No Reply', 'Bad Fit', 'Good Fit', 'Interested', 'Converted')),
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS offers (
  ...
  value DECIMAL(12,2) DEFAULT 0,
  probability INTEGER DEFAULT 0 CHECK (probability >= 0 AND probability <= 100),
  log_quality_rating INTEGER DEFAULT 0 CHECK (log_quality_rating >= 0 AND log_quality_rating <= 5),
  bad_fit_good_fit TEXT DEFAULT 'Pending' CHECK (bad_fit_good_fit IN ('Pending', 'Bad Fit', 'Good Fit', 'Not Interested', 'Interested')),
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- ============================================
-- LEAD FINANCIALS TABLE
-- Stores financial/contract information for leads
-- ============================================
