...
function extractEmail(text) {
    ...
    const validEmail = matches.find(email => !email.endsWith('.png') &&
        !email.endsWith('.jpg') &&
        !email.endsWith('.gif') &&
...
function cleanBusinessName(title) {
    ...
    if (/Log in|Sign up|Welcome back/i.test(name))
        return '';
    return name;
}
/**
 ...
 */
async function extractBusinessesFromResults(results) {
    const apiKey = process.env.GEMINI_API_KEY;
    ...
    const contentToAnalyze = results.map((r, i) => `Result ${i + 1}:
       URL: ${r.url}
       Title: ${r.title}
       Description: ${r.description}
       Content Snippet: ${(r.content || '').substring(0, 2000)}`).join('\n\n----------------\n\n');
    try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:generateContent?key=${apiKey}`, {
            method: 'POST',
...
export async function searchWithJina(rawQuery) {
    console.log('\n' + '='.repeat(50));
    ...
    while (allBusinesses.size < targetCount && attempts < maxAttempts) {
        ...
        if (extractedBusinesses.length === 0) {
            ...
            for (const result of results) {
                if (allBusinesses.size >= targetCount)
                    break;
                if (!result.url)
                    continue;
        ...
        for (const business of extractedBusinesses) {
            ...
            if (allBusinesses.has(domain))
                continue;
            if (!business.name || business.name === 'Unknown')
                continue;
